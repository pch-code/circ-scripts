...
getopt ('icso'); # i circ rna regions counts (Total rna) dir, c circ counts, s spliced size file, o bed output
...
foreach $key(sort {$circRNAs{$a}[2] cmp $circRNAs{$b}[2] or $circRNAs{$a}[3] <=> $circRNAs{$b}[3]} keys %circRNAs)
{
    ...
    $d = "$key\t";
    $d2 = "$key\t";
    #get length normalized TotalRNA read count estimate in 180bp of exonic length (given median insert size 210 and 15bp overlap for chimeric read detection; 210 - 15*2 = 180)
    for($i=0;$i<$ss;$i++)
    {
        if($elen==0){$NormCount = 0;}
        else
        {
            $NormCount = ${$circRNAs{$key}[1]}[$i]*180/$elen;
        }
